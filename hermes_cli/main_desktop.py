...
def cmd_gui(args: argparse.Namespace):
    ...
    if source_mode:
        ...
        try:
            ...
            raise SystemExit(1) from exc
        # `desktop.electron_flags` used to reach only the packaged path below, so a
        # `--source` launch silently ran on a different Ozone backend than the one
        # the user configured — and on a Wayland-capable host (WSLg) that drops CJK
        # input entirely, because the X11-only IM bridge goes away with it. Mirror
        # the packaged argv exactly (the packaged branch extends the same list).
        launch_command = [str(executable), ".", *config_electron_flags]
    else:
        if packaged_executable is None:
