...
def _prepare_source_launch_tree(tmp_path: Path) -> tuple[Path, Path]:
    """Desktop source tree plus the prepared Electron runtime a ``--source`` launch runs.

    The source launch no longer goes through ``npm exec``: it reads the installed
    Electron package's ``path.txt`` and starts that executable directly, so a
    source launch and a packaged launch differ only by argv.
    """
    root = _make_desktop_tree(tmp_path)
    desktop = root / "apps" / "desktop"
    (desktop / "dist").mkdir()
    ...
    (electron / "dist").mkdir(parents=True)
    (electron / "package.json").write_text("{}", encoding="utf-8")
    executable = electron / "dist" / "electron"
    executable.touch()
    (electron / "path.txt").write_text(executable.name + "\n", encoding="utf-8-sig")
...
def test_gui_source_launch_applies_configured_electron_flags(tmp_path, monkeypatch):
    ...
    """
    root, executable = _prepare_source_launch_tree(tmp_path)
    monkeypatch.setattr(cli_main, "PROJECT_ROOT", root)
    monkeypatch.setattr(main_desktop, "_desktop_macos_relaunchable_fixup", lambda *a, **kw: None)
    monkeypatch.setattr(main_desktop, "_register_linux_desktop_entry", lambda **kw: None)
    ...
    launched = calls[0][0]
    assert launched == [str(executable), ".", "--ozone-platform=x11"]
